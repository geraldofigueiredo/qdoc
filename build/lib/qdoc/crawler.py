...
class DocumentationCrawler:
    ...
    async def crawl_recursive(self, start_url: str, service: str, depth: int):
        """Crawl URLs recursively up to a certain depth, staying within the start_url scope."""
        queue = [(start_url, 0)]
        ...
        while queue:
            ...
            if current_depth < depth:
                for link in links:
                    # Filter: Only follow links that belong to the same base URL scope
                    if link.startswith(start_url) and link not in self.visited_urls:
